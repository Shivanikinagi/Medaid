...
def _build_triage_prompt(symptoms_text: str, report_summary: str = "", past_history: Optional[List[str]] = None, location: str = "") -> str:
    ...
    past_history_text = "No past medical history provided."
    if past_history and isinstance(past_history, list) and len(past_history) > 0:
        past_history_text = ", ".join(past_history)

    prompt = f"""
You are a village healthcare worker who knows local languages and speaks in simple, easy-to-understand terms. Provide a single JSON object only with the exact keys:
- possible_conditions: list of objects with "disease" (simple local term) and "confidence" (float 0-1)
- risk_level: one of ["Low","Medium","High","Emergency"]
- risk_proba: float 0-1
- reason: very short explanation (1-2 simple sentences using everyday words)
- recommendations: list of very practical, actionable steps (max 5, using simple everyday language, each starting with a verb and being a complete action step)

Patient info:
Symptoms: {symptoms_text}
Report summary: {report_summary}
Past medical history: {past_history_text}
Location: {location}

Instructions:
1. Use simple, everyday language that a rural person can easily understand
2. Avoid medical jargon - use common terms
3. Keep explanations very brief and practical
4. Make recommendations very specific and actionable, starting with a verb
5. Format each recommendation as a clear action step with bullet points
6. If risk is Emergency, clearly say "Go to hospital NOW" or "Call 108"
7. Number recommendations clearly in your mind (1, 2, 3, etc.) but don't include numbers in the JSON

Example good response:
{{"possible_conditions": [{{"disease": "Common Cold", "confidence": 0.8}}, {{"disease": "Flu", "confidence": 0.6}}], "risk_level": "Low", "risk_proba": 0.2, "reason": "Fever and cough are usually signs of common cold or flu. Most people get better with rest and home care.", "recommendations": ["• Rest and sleep well", "• Drink plenty of water", "• Take paracetamol for fever", "• Gargle with warm salt water", "• See doctor if fever doesn't improve in 3 days"]}}
...
def integrate_report_and_run_assessment(
    ...
    try:
        print(f"integrate_report_and_run_assessment called with:")
        print(f"  report_data: {report_data}")
        print(f"  user_inputs: {user_inputs}")
        print(f"  user_profile: {user_profile}")
        
        # Get symptoms_text from current_symptoms
        symptoms_text = (user_inputs or {}).get("current_symptoms", "") or ""
        ...
        # Check for emergency conditions in report data
        if isinstance(report_data, dict):
            try:
        ...
        # Create a comprehensive report summary that includes all relevant data
        report_summary = ""
        if isinstance(report_data, dict):
        ...
        past_history = []
        if user_profile and "past_history" in user_profile:
            past_history_data = user_profile["past_history"]
            if isinstance(past_history_data, list):
                past_history = past_history_data
            elif isinstance(past_history_data, dict):
                # If it's a dict, extract the keys or values
                past_history = list(past_history_data.keys()) if past_history_data else []
        
        print(f"  past_history: {past_history}")

        # Combine all available information for the assessment
        # Include past history, current symptoms, and report data
        comprehensive_info = f"""
Current Symptoms: {symptoms_text}
Past Medical History: {', '.join(past_history) if past_history else 'None'}
Report Data: {report_summary}
"""
        
        print(f"  comprehensive_info: {comprehensive_info}")

        # Call LLM with all available data
        llm_out = llm_predict_assessment(
            comprehensive_info.strip(), 
            report_summary, 
            past_history,
